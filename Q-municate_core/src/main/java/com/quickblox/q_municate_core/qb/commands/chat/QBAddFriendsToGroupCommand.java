...
public class QBAddFriendsToGroupCommand extends ServiceCommand {
    ...
    public Bundle perform(Bundle extras) throws Exception {
        ...
        QBChatDialog  QBChatDialog  = multiChatHelper.addUsersToDialog(dialogId, friendIdsList);

        if (QBChatDialog  != null) {
            Dialog dialog = ChatUtils.createLocalDialog(QBChatDialog );
            DataManager.getInstance().getDialogDataManager().update(dialog);
            DbUtils.saveDialogsOccupants(DataManager.getInstance(), QBChatDialog , true);
        }

        Bundle returnedBundle = new Bundle();
        returnedBundle.putSerializable(QBServiceConsts.EXTRA_DIALOG, QBChatDialog );
